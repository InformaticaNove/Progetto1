...
#+
PUBLIC FUNCTION form_uidialogdata_record1_clear()
...
PUBLIC FUNCTION form_uidialogdata_record1_fetchAll(p_where, p_detailList)
    ...
    CALL form_uidialogdata_record1_clear()
    CALL form_uidata.form_uidata_record1_getKeys(m_record1_arrKeyRec, p_where, p_detailList) RETURNING errNo
...
{<BLOCK Name="fct.record1_fetchAllDisplay">}
#+ Fetch the data set for 'record1'
#+
#+ @param p_where        The WHERE part to apply
#+ @param p_detailList   List of details on whom a CONSTRUCT was done
PUBLIC FUNCTION form_uidialogdata_record1_fetchAllDisplay(p_where, p_detailList)
    DEFINE p_where STRING
    DEFINE p_detailList STRING
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchAllDisplay.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchAllDisplay.init">} {</POINT>}
    CALL form_uidialogdata_record1_clear()
    CALL form_uidata.form_uidata_record1_getKeys(m_record1_arrKeyRec, p_where, p_detailList) RETURNING errNo
    IF errNo < ERROR_SQL_SUCCESS THEN
        LET m_record1_keyIndex = 0
...
#+
FUNCTION form_uidialogdata_record1_fetchRow()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchRow.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchRow.init">} {</POINT>}
    IF m_record1_keyIndex > 0 THEN
        CALL form_uidata.form_uidata_record1_getDataByKey(m_record1_arrKeyRec[m_record1_keyIndex].collegamento_utente) RETURNING errNo, m_record1_arrRecGrid[1].*, m_record1_arrRecDB[1].*
        IF errNo < ERROR_SQL_SUCCESS THEN
            CALL libdbapp_utilMsgStatusBar(errNo, libdbapp_formatSqlErrorMsg(errNo))
        END IF
    END IF
...
#+
FUNCTION form_uidialogdata_record1_fetchDetails()
    DEFINE errNo INTEGER
...
{<BLOCK Name="fct.record1_fetchDetailsDisplay">}
#+ Fetch details data for 'record1'
#+
FUNCTION form_uidialogdata_record1_fetchDetailsDisplay()
    DEFINE errNo INTEGER
    {<POINT Name="fct.record1_fetchDetailsDisplay.define">} {</POINT>}
    {<POINT Name="fct.record1_fetchDetailsDisplay.init">} {</POINT>}
END FUNCTION
{</BLOCK>} --fct.record1_fetchDetailsDisplay

{<BLOCK Name="fct.record1_setDefaultValues">}
#+ Initialize 'record1' with default values
#+
#+ @param p_dialog     The current DIALOG
PUBLIC FUNCTION form_uidialogdata_record1_setDefaultValues(p_dialog)
    ...
    IF form_events.m_DataEvent_record1_OnDefaultValues IS NOT NULL THEN
        CALL form_events.m_DataEvent_record1_OnDefaultValues(m_record1_arrRecGrid[1].*)
...
PUBLIC FUNCTION form_uidialogdata_record1_seek(p_record1_br_uk)
    ...
    IF p_record1_br_uk.collegamento_utente IS NULL THEN
        LET idxReturn = 0
    END IF

    --If the seeking values match the current row, then no need to seek and return the current index
    IF m_record1_keyIndex > 0 THEN
        IF p_record1_br_uk.collegamento_utente == m_record1_arrKeyRec[m_record1_keyIndex].collegamento_utente THEN
    ...
    FOR i = 1 TO s
        IF p_record1_br_uk.collegamento_utente == m_record1_arrKeyRec[i].collegamento_utente THEN
            LET idxExactMatch = i
            EXIT FOR
        END IF
    END FOR

...
PUBLIC FUNCTION form_ui_uiOpenForm(p_whereRelation, p_uiSettings)
    ...
    CALL form_ui_uiOpenFormByKey(p_whereRelation, p_uiSettings.*, l_record1_br_uk.*) RETURNING errNo, actionNo, l_record1_br_uk.*
...
PUBLIC FUNCTION form_ui_uiOpenFormByKey(p_whereRelation, p_uiSettings, p_br_uk)
    DEFINE p_whereRelation STRING
    DEFINE p_uiSettings UISettings_Type
    ...
    CALL form_uidialog.form_uidialog_preloadModule()
    OPEN WINDOW w_form WITH FORM 'form' ATTRIBUTES(STYLE=NVL(l_windowStyle, "main_dbapp"))
    --Initialize UI settings with default form UI settings
    CALL form_ui_initializeDefaultUISettings() RETURNING l_defaultUISettings.*
    ...
    CALL form_events.form_registerDlgEvents()
    CALL mydatabase_events.mydatabase_registerDbxEvents()

    --Load actions in the ToolBar and the TopMenu
    IF NOT libdbapp_isMobile() THEN
        CALL form_ui_uiInitializeDefaultActions(l_actionList)
    ...
    IF form_events.m_DlgEvent_OnOpenForm IS NOT NULL THEN
        CALL form_events.m_DlgEvent_OnOpenForm(w.getForm())
    END IF
    ...
    IF m_uiSettings.openMode != C_MODE_SEARCH AND m_uiSettings.openMode != C_MODE_EMPTY THEN
        ...
        LET m_record1_keyIndex = form_uidialogdata_record1_seek(p_br_uk.*)
    END IF
    --Call the UI automaton
    CALL form_ui_uiAutomaton(m_uiSettings.*) RETURNING errNo, actionNo
    INITIALIZE l_record1_br_uk.* TO NULL
    IF m_record1_keyIndex > 0 THEN
        LET l_record1_br_uk.collegamento_utente = m_record1_arrKeyRec[m_record1_keyIndex].collegamento_utente
    END IF
    CLOSE WINDOW w_form
...
PRIVATE FUNCTION form_ui_uiAutomaton(p_uiSettings)
    DEFINE p_uiSettings UISettings_Type
    ...
    WHILE runningAutomaton
        CASE l_state
            ...
            WHEN C_MODE_DISPLAY
                ...
                CALL form_ui_uiDisplay(prevState, prevAction, l_state) RETURNING errNo, actionNo
            ...
            WHEN C_MODE_MODIFY
                ...
                CALL form_ui_uiInput(prevState, prevAction, l_state) RETURNING errNo, actionNo
            ...
            WHEN C_MODE_SEARCH
                ...
                CALL form_ui_uiConstruct(prevState, prevAction, l_state) RETURNING errNo, actionNo
            ...
            WHEN C_MODE_EMPTY
                ...
                CALL form_ui_uiEmpty(prevState, prevAction, l_state) RETURNING errNo, actionNo
...
FUNCTION form_ui_uiDisplay(p_prevState, p_prevAction, p_state)
    ...
    DIALOG ATTRIBUTES(UNBUFFERED, FIELD ORDER FORM)
        SUBDIALOG form_uidialog.form_uidialog_record1_uiDisplay
        ...
        ON ACTION new
            ...
            IF form_events.m_DlgCtrlInstruction IS NULL THEN
                ...
                IF form_events.m_DlgEvent_OnActionNew IS NOT NULL THEN
                    CALL form_events.m_DlgEvent_OnActionNew(DIALOG, m_state, form_events.m_DlgCtrlInstruction)
                        RETURNING form_events.m_DlgCtrlInstruction
                END IF
                ...
                CASE form_events.m_DlgCtrlInstruction
                    WHEN libdbappEvents.ACCEPT_DIALOG
                        ACCEPT DIALOG
                    WHEN libdbappEvents.CANCEL_INSERT
                        CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "OnActionNew", NULL), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                    WHEN libdbappEvents.CONTINUE_DIALOG
                        CONTINUE DIALOG
                    WHEN libdbappEvents.EXIT_DIALOG
                        EXIT DIALOG
        ...
        ON ACTION query
            ...
            LET form_events.m_DlgCtrlInstruction = libdbappEvents.ACCEPT_DIALOG
            IF form_events.m_DlgEvent_OnActionQuery IS NOT NULL THEN
                CALL form_events.m_DlgEvent_OnActionQuery(DIALOG, m_state, form_events.m_DlgCtrlInstruction)
                    RETURNING form_events.m_DlgCtrlInstruction
            END IF
            ...
            CASE form_events.m_DlgCtrlInstruction
                WHEN libdbappEvents.ACCEPT_DIALOG
                    ACCEPT DIALOG
                WHEN libdbappEvents.CANCEL_INSERT
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "OnActionQuery", NULL), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CONTINUE_DIALOG
                    CONTINUE DIALOG
                WHEN libdbappEvents.EXIT_DIALOG
                    EXIT DIALOG
        ...
        ON ACTION accept
            ...
            LET form_events.m_DlgCtrlInstruction = libdbappEvents.ACCEPT_DIALOG
            IF form_events.m_DlgEvent_OnActionAccept IS NOT NULL THEN
                CALL form_events.m_DlgEvent_OnActionAccept(DIALOG, m_state, form_events.m_DlgCtrlInstruction)
                    RETURNING form_events.m_DlgCtrlInstruction
            END IF
            ...
            CASE form_events.m_DlgCtrlInstruction
                WHEN libdbappEvents.ACCEPT_DIALOG
                    ACCEPT DIALOG
                WHEN libdbappEvents.CANCEL_INSERT
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "OnActionAccept", NULL), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CONTINUE_DIALOG
                    CONTINUE DIALOG
                WHEN libdbappEvents.EXIT_DIALOG
                    EXIT DIALOG
        ...
        ON ACTION cancel
            ...
            IF form_events.m_DlgEvent_OnActionCancel IS NOT NULL THEN
                CALL form_events.m_DlgEvent_OnActionCancel(DIALOG, m_state, form_events.m_DlgCtrlInstruction)
                    RETURNING form_events.m_DlgCtrlInstruction
            END IF
            ...
            CASE form_events.m_DlgCtrlInstruction
                WHEN libdbappEvents.ACCEPT_DIALOG
                    ACCEPT DIALOG
                WHEN libdbappEvents.CANCEL_INSERT
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "OnActionCancel", NULL), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CONTINUE_DIALOG
                    CONTINUE DIALOG
                WHEN libdbappEvents.EXIT_DIALOG
                    EXIT DIALOG
        ...
        ON ACTION close
            ...
            LET form_events.m_DlgCtrlInstruction = libdbappEvents.EXIT_DIALOG
            IF form_events.m_DlgEvent_OnActionClose IS NOT NULL THEN
                CALL form_events.m_DlgEvent_OnActionClose(DIALOG, m_state, form_events.m_DlgCtrlInstruction)
                    RETURNING form_events.m_DlgCtrlInstruction
            END IF
            ...
            CASE form_events.m_DlgCtrlInstruction
                WHEN libdbappEvents.ACCEPT_DIALOG
                    ACCEPT DIALOG
                WHEN libdbappEvents.CANCEL_INSERT
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "OnActionClose", NULL), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CONTINUE_DIALOG
                    CONTINUE DIALOG
                WHEN libdbappEvents.EXIT_DIALOG
                    EXIT DIALOG
        ...
        ON ACTION exit
            ...
            LET form_events.m_DlgCtrlInstruction = libdbappEvents.EXIT_DIALOG
            IF form_events.m_DlgEvent_OnActionExit IS NOT NULL THEN
                CALL form_events.m_DlgEvent_OnActionExit(DIALOG, m_state, form_events.m_DlgCtrlInstruction)
                    RETURNING form_events.m_DlgCtrlInstruction
            END IF
            ...
            CASE form_events.m_DlgCtrlInstruction
                WHEN libdbappEvents.ACCEPT_DIALOG
                    ACCEPT DIALOG
                WHEN libdbappEvents.CANCEL_INSERT
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "OnActionExit", NULL), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CONTINUE_DIALOG
                    CONTINUE DIALOG
                WHEN libdbappEvents.EXIT_DIALOG
                    EXIT DIALOG
        ...
        BEFORE DIALOG
            ...
            LET form_events.m_DlgCtrlInstruction = libdbappEvents.CONTINUE_DIALOG
            IF form_events.m_DlgEvent_Before_BrowseDialog IS NOT NULL THEN
                CALL form_events.m_DlgEvent_Before_BrowseDialog(DIALOG, m_state, form_events.m_DlgCtrlInstruction, m_nextField)
                    RETURNING form_events.m_DlgCtrlInstruction, m_nextField
            END IF
            ...
            CASE form_events.m_DlgCtrlInstruction
                WHEN libdbappEvents.ACCEPT_DIALOG
                    ACCEPT DIALOG
                WHEN libdbappEvents.CANCEL_INSERT
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "Before_BrowseDialog", NULL), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CONTINUE_DIALOG
                    CONTINUE DIALOG
                WHEN libdbappEvents.EXIT_DIALOG
                    EXIT DIALOG
        ...
        AFTER DIALOG
            {<POINT Name="fct.uiDisplay.dlg.afterDialog">} {</POINT>}
            INITIALIZE form_events.m_DlgCtrlInstruction TO NULL
            IF form_events.m_DlgEvent_After_BrowseDialog IS NOT NULL THEN
                CALL form_events.m_DlgEvent_After_BrowseDialog(DIALOG, m_state, form_events.m_DlgCtrlInstruction)
                    RETURNING form_events.m_DlgCtrlInstruction
            END IF

            CASE form_events.m_DlgCtrlInstruction
                WHEN libdbappEvents.ACCEPT_DIALOG
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "After_BrowseDialog", NULL), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CANCEL_INSERT
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "After_BrowseDialog", NULL), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CONTINUE_DIALOG
                    CONTINUE DIALOG
...
#+ @return           Error number, 0-success, other-error
#+ @return           The next performed action
FUNCTION form_ui_uiInput(p_prevState, p_prevAction, p_state)
    DEFINE p_prevState SMALLINT
    DEFINE p_prevAction SMALLINT
    DEFINE p_state SMALLINT
    ...
    DIALOG ATTRIBUTES(UNBUFFERED, FIELD ORDER FORM)
        SUBDIALOG form_uidialog.form_uidialog_record1_uiInput
        ...
        ON ACTION cancel
            ...
            CALL form_ui_validateCRUDOperationWrapper(DIALOG, m_subDialog, m_state, C_ACTION_CANCEL)
        ...
        BEFORE DIALOG
            ...
            END IF
            LET form_events.m_DlgCtrlInstruction = libdbappEvents.CONTINUE_DIALOG
            IF form_events.m_DlgEvent_Before_EditDialog IS NOT NULL THEN
                CALL form_events.m_DlgEvent_Before_EditDialog(DIALOG, m_state, form_events.m_DlgCtrlInstruction, m_nextField)
                    RETURNING form_events.m_DlgCtrlInstruction, m_nextField
            END IF
            ...
            CASE form_events.m_DlgCtrlInstruction
                WHEN libdbappEvents.ACCEPT_DIALOG
                    ACCEPT DIALOG
                WHEN libdbappEvents.CANCEL_INSERT
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "Before_EditDialog", NULL), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CONTINUE_DIALOG
                    CONTINUE DIALOG
                WHEN libdbappEvents.EXIT_DIALOG
                    EXIT DIALOG
        ...
        AFTER DIALOG
            {<POINT Name="fct.uiInput.dlg.afterDialog">} {</POINT>}
            INITIALIZE form_events.m_DlgCtrlInstruction TO NULL
            IF form_events.m_DlgEvent_After_EditDialog IS NOT NULL THEN
                CALL form_events.m_DlgEvent_After_EditDialog(DIALOG, m_state, form_events.m_DlgCtrlInstruction)
                    RETURNING form_events.m_DlgCtrlInstruction
            END IF

            CASE form_events.m_DlgCtrlInstruction
                WHEN libdbappEvents.ACCEPT_DIALOG
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "After_EditDialog", NULL), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CANCEL_INSERT
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "After_EditDialog", NULL), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CONTINUE_DIALOG
                    CONTINUE DIALOG
...
#+ @return             Error number, 0-success, other-error
#+ @return             The next performed action
FUNCTION form_ui_uiConstruct(p_prevState, p_prevAction, p_state)
    DEFINE p_prevState SMALLINT
    DEFINE p_prevAction SMALLINT
    DEFINE p_state SMALLINT
    ...
    DIALOG ATTRIBUTES(UNBUFFERED, FIELD ORDER FORM)
        SUBDIALOG form_uidialog.form_uidialog_record1_uiConstruct
        ...
        BEFORE DIALOG
            ...
            INITIALIZE form_events.m_DlgCtrlInstruction TO NULL
            IF form_events.m_DlgEvent_Before_SearchDialog IS NOT NULL THEN
                CALL form_events.m_DlgEvent_Before_SearchDialog(DIALOG, m_state, form_events.m_DlgCtrlInstruction)
                    RETURNING form_events.m_DlgCtrlInstruction
            END IF
            CASE form_events.m_DlgCtrlInstruction
                WHEN libdbappEvents.ACCEPT_DIALOG
                    ACCEPT DIALOG
                WHEN libdbappEvents.CANCEL_INSERT
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "Before_SearchDialog", NULL), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CONTINUE_DIALOG
                    CONTINUE DIALOG
                WHEN libdbappEvents.EXIT_DIALOG
                    EXIT DIALOG
        ...
        AFTER DIALOG
            ...
            INITIALIZE form_events.m_DlgCtrlInstruction TO NULL
            IF form_events.m_DlgEvent_After_SearchDialog IS NOT NULL THEN
                CALL form_events.m_DlgEvent_After_SearchDialog(DIALOG, m_state, form_events.m_DlgCtrlInstruction, m_where, m_detailList)
                    RETURNING form_events.m_DlgCtrlInstruction, m_where, m_detailList
            END IF

            CASE form_events.m_DlgCtrlInstruction
                WHEN libdbappEvents.ACCEPT_DIALOG
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "After_SearchDialog", NULL), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CANCEL_INSERT
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "After_SearchDialog", NULL), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CONTINUE_DIALOG
                    CONTINUE DIALOG
...
#+ @return             Error number, 0-success, other-error
#+ @return             The next performed action
FUNCTION form_ui_uiEmpty(p_prevState, p_prevAction, p_state)
    DEFINE p_prevState SMALLINT
    DEFINE p_prevAction SMALLINT
    DEFINE p_state SMALLINT
    DEFINE errNo INTEGER
    ...
    MENU
        BEFORE MENU
            ...
            END IF
            INITIALIZE form_events.m_DlgCtrlInstruction TO NULL
            IF form_events.m_DlgEvent_Before_EmptyDialog IS NOT NULL THEN
                CALL form_events.m_DlgEvent_Before_EmptyDialog(DIALOG, m_state, form_events.m_DlgCtrlInstruction)
                    RETURNING form_events.m_DlgCtrlInstruction
            END IF

            CASE form_events.m_DlgCtrlInstruction
                WHEN libdbappEvents.CANCEL_INSERT
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "Before_EmptyDialog", "EMPTY"), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CONTINUE_MENU
                    CONTINUE MENU
                WHEN libdbappEvents.EXIT_MENU
                    EXIT MENU
        ...
        ON ACTION new
            ...
            CASE form_events.m_DlgCtrlInstruction
                ...
                WHEN libdbappEvents.EXIT_MENU
                    EXIT MENU
                WHEN libdbappEvents.ACCEPT_DIALOG
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "BeforeInsert", "EMPTY"), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CONTINUE_DIALOG
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "BeforeInsert", "EMPTY"), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.EXIT_DIALOG
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "BeforeInsert", "EMPTY"), libdbapp_canShowDialogBox(), libdbapp_isFatal())
            ...
            IF form_events.m_DlgCtrlInstruction IS NULL THEN
                LET m_actionNo = C_ACTION_NEW
                LET form_events.m_DlgCtrlInstruction = libdbappEvents.EXIT_MENU
                IF form_events.m_DlgEvent_OnActionNew IS NOT NULL THEN
                    CALL form_events.m_DlgEvent_OnActionNew(DIALOG, m_state, form_events.m_DlgCtrlInstruction)
                        RETURNING form_events.m_DlgCtrlInstruction
                END IF
                ...
                CASE form_events.m_DlgCtrlInstruction
                    WHEN libdbappEvents.CANCEL_INSERT
                        CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "OnActionNew", "EMPTY"), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                    WHEN libdbappEvents.CONTINUE_MENU
                        CONTINUE MENU
                    WHEN libdbappEvents.EXIT_MENU
                        EXIT MENU
        ...
        ON ACTION query
            ...
            CASE form_events.m_DlgCtrlInstruction
                WHEN libdbappEvents.CANCEL_INSERT
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "OnActionQuery", "EMPTY"), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CONTINUE_MENU
                    CONTINUE MENU
                WHEN libdbappEvents.EXIT_MENU
                    EXIT MENU
        ...
        ON ACTION close
            ...
            CASE form_events.m_DlgCtrlInstruction
                WHEN libdbappEvents.CANCEL_INSERT
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "OnActionClose", "EMPTY"), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CONTINUE_MENU
                    CONTINUE MENU
                WHEN libdbappEvents.EXIT_MENU
                    EXIT MENU
        ...
        ON ACTION exit
            ...
            CASE form_events.m_DlgCtrlInstruction
                WHEN libdbappEvents.CANCEL_INSERT
                    CALL libdbapp_showError(__FILE__, __LINE__, "GS-13173", libdbapp_makeMessage(form_events.m_DlgCtrlInstruction, "OnActionExit", "EMPTY"), libdbapp_canShowDialogBox(), libdbapp_isFatal())
                WHEN libdbappEvents.CONTINUE_MENU
                    CONTINUE MENU
                WHEN libdbappEvents.EXIT_MENU
                    EXIT MENU
...
PUBLIC FUNCTION form_ui_uiInitializeDefaultActions(p_actionList)
...
PUBLIC FUNCTION form_ui_initializeDefaultUISettings()
...
FUNCTION form_ui_validateCRUDOperationWrapper(p_dialog, p_subdialog_name, p_state, p_action)
    ...
    CASE p_subdialog_name
        WHEN "record1"
            CALL form_uidialog.form_uidialog_record1_validateCRUDOperation(p_dialog, p_state, p_action)
                RETURNING l_internalAction
    END CASE
